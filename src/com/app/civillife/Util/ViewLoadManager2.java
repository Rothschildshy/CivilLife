...
public class ViewLoadManager2 {
	...
	public ViewLoadManager2(Context context) {
		super();
		this.context = context;
		taskCollection = new HashSet<ViewLoadManager2.ImageLoadTask>();
	}

	private static LruCache<String, Bitmap> BitmapMemoryCache = new LruCache<String, Bitmap>(
			maxCacheSize) {
		@Override
	 ...
	 *            显示图片的ImageView
	 */
	public void setImageView(IMAGE_LOAD_TYPE loadType, int imageResourceID,
			ImageView imageView) {
		if (loadType == IMAGE_LOAD_TYPE.FILE_RESOURCE_ID) {
			...
			try {
				...
				Qlog.e(TAG, "Can find the imageID of " + imageResourceID);
				e.printStackTrace();
			}
			// 默认图片
